  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-brown-100 bg-white shadow-sm transition-shadow hover:shadow-lg">
      <div className="relative h-44 overflow-hidden">
        ...
        />
        <span className="absolute top-3 left-3 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-brown-700">
          {course.level}
        </span>
      </div>
      <div className="flex flex-1 flex-col p-5">
        ...
        <ul className="mt-4 space-y-1.5">
          {course.highlights.slice(0, 3).map((item) => (
            <li key={item} className="flex items-start gap-2 text-sm text-ink/80">
              <svg viewBox="0 0 20 20" className="mt-0.5 h-4 w-4 shrink-0 text-beige-500" fill="currentColor">
        ...
        <div className="mt-5 flex items-end justify-between border-t border-brown-100 pt-4">
          <div>
            {course.oldPrice && (
